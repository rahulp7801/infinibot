...
async function music_message(message, mapKey) {
    ...
    queueTryPlayNext(mapKey, (title, uploader, thumbnail, link, vidlen)=>{
        ...
        const embed = new Discord.MessageEmbed().setColor('#ff0000').setFooter('Made possible by the DJFlame Team', 'https://i.ibb.co/YhG2Xj4/DJFlame-Logo.png').setDescription('['+title+']('+link+')').setThumbnail(thumbnail).setAuthor('Added to queue', servericon).addFields({name: 'Uploader', value: uploader, inline:true}, {name: 'Length', value: vidlen, inline: true})
...
async function getYoutubeVideoData(str, isretry=false) {
    try {
        ...
        if (data && 'id' in data && 'title' in data) {
            YT_CACHE[str] = {id:data.id, title:data.title, uploader: data.uploader, thumbnail: data.thumbnail.hqDefault, vidlen: data.timestamp, link: data.url, created: Math.floor(new Date())};
        }
        return data;
    } catch (e) {
        if (!isretry) {
